...
1. **Regime** — SPY daily **10-EMA crosses below 20-EMA** (was above on the prior
   bar → fresh bearish cross) ⇒ **close all** open campaigns. `exitReason = regime`.
2. **Emergency** — snapshot stock ≤ `entry − 3*ATR` ⇒ close. `exitReason = emergency`.
3. **Stop** — snapshot stock ≤ `entry − 1*ATR` ⇒ close. `exitReason = stop`.
   *(Order 2 before 3: the more extreme −3 ATR breach is tagged `emergency`;
   a breach between −1 and −3 ATR is tagged `stop`.)*
4. **Time-roll** — current leg **DTE ≤ 7** ⇒ roll to the time-roll target (§7);
   if no liquid ≥0.60-delta contract exists, **close**. `exitReason = dte_close`.
5. **Winner roll-up** — stock has reached a **new** `entry + k*ATR` threshold
   (k = `rollUpStepsTaken + 1`) ⇒ **roll up and out** to the winner target (§7),
   then `rollUpStepsTaken += 1`. Each additional +1 ATR step rolls again.
...
- **Entry stock price / ATR:** FMP `/stable` (5-min intraday for the entry moment;
  daily history for ATR(14)). Same as position-sizer.
- **Option delta / live mark:** `tradier-proxy` (greeks).
- **SPY EMAs:** FMP SPY daily closes → 10-EMA and 20-EMA.
...
- **"Rapid intraday" emergency** is best-effort: with a few snapshots/day it's
  ...
  snapshots is missed.
- **Snapshot cadence:** GitHub Action cron is approximate (every ~30–60 min in
  market hours), not to-the-second.
...
- **Settings** — paper-account balance, risk %, thresholds (writes `config.json`),
  and API keys + GitHub PAT (localStorage only).
...
boundary, roll accounting (realized + unrealized chaining), and every scorecard
...
3. **Regime exit closes all** open campaigns immediately on the daily cross.
