...
(function () {
  ...
  function addSelected() {
    ...
    setHistory(history);
    msg.textContent = 'Added ' + e.ticker + ' ' + c.strike + 'C ' + exp + ' x' + contracts;
  ...
  async function tick() {
    ...
    for (var i = 0; i < positions.length; i++) {
      ...
      try {
        ...
        if (ctx._updatedHWM != null) { positions[i].premiumHWM = ctx._updatedHWM; changed = true; }
        if (action.type !== 'none') {
          var res = E.applyAction(positions[i], action, ctx);
  ...
  async function closeCampaign(id) {
    ...
    for (var i = 0; i < positions.length; i++) {
      if (positions[i].id === id && positions[i].status === 'open') {
        ...
        if (mark == null) { try { mark = (await provider().getOptionQuote(E.occSymbol(positions[i].ticker, positions[i].legs[positions[i].legs.length - 1].expiration, 'C', positions[i].legs[positions[i].legs.length - 1].strike))).mark; } catch (e) { setStatus('need a mark to close'); return; } }
  ...
  function renderWatchlist() {
    ...
    w.forEach(function (it) {
      ...
      var tr = document.createElement('tr');
      tr.innerHTML = '<td><strong>' + it.ticker + '</strong></td><td>' + (it.last != null ? fmt2(it.last) : '-') + '</td><td>' + pct + '</td><td>' + (it.atr != null ? fmt2(it.atr) : '-') + '</td><td>' + sug + '</td><td><button class="ghost" data-chain="' + it.ticker + '" style="padding:4px 10px">Chain</button> <button class="btn" data-buy="' + it.ticker + '" style="padding:4px 12px">Buy</button></td><td><button class="danger" data-rm="' + it.ticker + '">x</button></td>';
      tb.appendChild(tr);
    });
